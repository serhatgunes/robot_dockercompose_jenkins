...
node {
    ...
    stage('Docker Setup') {
        parallel(
          "Start Compose": {
    	    ...
    	    cmd_exec('docker-compose --log-level INFO -f docker-compose.yml up -d')
          },
          "Build Image": {
    ...
    stage('Execute') {
       ...
       if (isUnix()) {
                ...
                sh './wait-for-grid.sh && docker exec test-execution pabot --pabotlib --processes 2 --verbose --argumentfile1 Firefox.args --argumentfile2 Chrome.args --outputdir ./reports  test/ui_tests.robot'
            }
        else {
